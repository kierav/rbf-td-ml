function [xy2] = repel(xy,bdy,corners,radius,vargin)
...
xyb(:,3) = radius([xyb(:,1),xyb(:,2)],vargin);   % Get radii at all points
...
for rounds = 1:NR   % Loop over the rounds of node pushing 
    ...
    for k = 1:NP        % Loop over rounds of node pushing
        ...
        for np = 1:li2  % Loop over all the nodes that are to be pushed
            ...
            xyn(2) = xyn(2)+push2;
            xyn(3)   = radius ([xyn(1),xyn(2)],vargin);% Get local grain radius
                                                % at new location
